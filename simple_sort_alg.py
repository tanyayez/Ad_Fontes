...
class City:
    ...
    def simulation(self):
        # sort rides by earl start

        self.map.sort(key=lambda ride: ride.e_st)
...
def main():
    city = City()
    city.read_from_file(files[4])
    city.simulation()
    res = city.validate()
    city.write_result('SimpleSortAlgRes/e_res.out')